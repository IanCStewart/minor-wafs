...
(function() {
    ...
    var location = {
        ...
        update: function() {
            ...
            geo_position_js.getCurrentPosition(set, debug.geoErrorHandler, {enableHighAccuracy: true});
        },
    ...
    var google = {
        generateMap: function(myOptions, canvasID) {
            ...
            for (var i = 0; i < locations.length; i++) {
                ...
                var marker = new google.maps.Marker({position: markerLatLng, map: config.map, icon: config.markerArray[i], title: locations[i][0]
                });
            }
            ...
            config.currentPositionMarker = new google.maps.Marker({position: kaartOpties.center, map: config.map, icon: positieMarker, title: 'U are here'});
